var b=(s,e,t)=>{const o=s[e];return o?typeof o=="function"?o():Promise.resolve(o):new Promise((a,r)=>{(typeof queueMicrotask=="function"?queueMicrotask:setTimeout)(r.bind(null,new Error("Unknown variable dynamic import: "+e+(e.split("/").length!==t?". Note that variables only represent file names one level deep.":""))))})};const _="0.27.0";let n=null,p=!1;const d=`
import numpy as np
...
`;async function m(){try{self.postMessage({type:"loading",message:"Loading Pyodide…"});const{loadPyodide:s}=await b(Object.assign({}),`../../https:/cdn.jsdelivr.net/pyodide/v${_}/full/pyodide.mjs`,8);self.postMessage({type:"loading",message:"Loading numpy + scipy…"}),n=await s(),await n.loadPackage(["numpy","scipy"]),self.postMessage({type:"loading",message:"Compiling FBA kernel…"}),await n.runPythonAsync(d),p=!0,self.postMessage({type:"ready",solver:"pyodide-scipy-highs",version:_,capabilities:["fba","pfba","fva","moma"]})}catch(s){self.postMessage({type:"error",error:`Pyodide init failed: ${s.message}`})}}const u={fba:"py_fba",pfba:"py_pfba",fva:"py_fva",moma:"py_moma"};self.onmessage=async function(s){const{jobId:e,method:t,model:o,options:a={}}=s.data;if(!p){self.postMessage({jobId:e,type:"error",error:"Pyodide not ready"});return}const r=u[t];if(!r){self.postMessage({jobId:e,type:"error",error:`Method '${t}' not supported in Pyodide tier`});return}try{n.globals.set("_model_json",JSON.stringify(o)),n.globals.set("_options_json",JSON.stringify(a));const i=await n.runPythonAsync(`${r}(_model_json, _options_json)`),l=JSON.parse(i);l._tier="pyodide",self.postMessage({jobId:e,type:"result",result:l})}catch(i){self.postMessage({jobId:e,type:"error",error:i.message})}};m();
